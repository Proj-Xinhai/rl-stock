...
class OnlyInstitutionalInvestorHelper(BasicPipelineHelper):
    def __init__(self):
        super(OnlyInstitutionalInvestorHelper, self).__init__()  # 繼承父類別的__init__()
        self.observation_space = spaces.Box(
            low=np.array([np.concatenate([np.zeros(1), -np.ones(7), np.array([-np.inf])])]),
            high=np.array([np.concatenate([np.ones(1), np.ones(7), np.array([np.inf])])]),
            shape=(1, 1 + 7 + 1),  # 1: 收盤價, 7: 法人買賣超, 1: 持有量
            dtype=np.float32
        )
...
EXPORT = OnlyInstitutionalInvestorHelper
DESCRIPT = 'institutional investor data only (normal_helper)'
